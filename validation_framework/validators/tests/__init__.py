...
modules = glob.glob(dirname(__file__) + "/tests/*.py")
...
class Validators:
    ...
    def register_validator(cls, validator_name: str, p_validator):
        ...
        file_logger.info(f'Distribution {validator_name} registered')
    ...
    def validator(cls, validator_name: str = None):
        ...
        def decorator(validator_class):
            ...
            if _validator_name in cls.active_validators:
                file_logger.error(f'Monitor {_validator_name} is already defined')
            else:
