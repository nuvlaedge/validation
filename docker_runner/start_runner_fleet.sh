...
n=2
for (( i = 0; i < n; i++ )); do
    docker run --detach --rm --name validation_runner_${i} --hostname validation_runner_$ --env RUNNER_TOKEN=$RUNNER_TOKEN --env ORGANIZATION=nuvlaedge  -v testbed_keys:/home/runner/.ssh/ validation-runner:0.0.1a1
done
